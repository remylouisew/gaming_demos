...
if __name__ == "__main__":
    ...
    for i in range( int(args['number_of_records']) ):
        ...
        second = str(random.randint(1,59)).zfill(2)
        #event_datetime = '{}-{}-{} {}:{}:{}'.format(year,month,day,hour,minute,second)
        event_datetime = '{}-{}-{}'.format(year,month,day)
        ...
        payload = {
            ...
            'weapon':       weapon,
            'blocksBlasted':random.randint(0,15),
            'powerUpsUsed': random.randint(0,10),
            'moves':        random.randint(1,1000),
            'coins':        random.randint(1,250),
            'rank':         random.randint(1,5000),
            'purchases':    random.random() * 5,
            'friends':      random.randint(1,300)
        }
